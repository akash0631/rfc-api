FUNCTION ZWM_APP_ARTICLE_SA_ST_DETAILS.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"  TABLES
*"      ET_DATA TYPE  ZWM_ST_SA_DATA_T OPTIONAL
*"----------------------------------------------------------------------
  ...
  DATA : LT_MARD  TYPE STANDARD TABLE OF MARD,
         ...
         LT_DATA  TYPE ZWM_ST_SA_DATA_T,
         LT_DATA2 TYPE ZWM_ST_SA_DATA_T.
  ...
  DATA : REF_DATA TYPE REF TO ZWM_ST_SA_DATA.
...
*       ls_data-msa = lv_tot.
    ...
    SELECT SINGLE matnr werks ekgrp dismm dispo plifz umlmc trame
      FROM MARC INTO LS_MARC WHERE MATNR = LS_LQUA-MATNR
                                                AND WERKS = IM_WERKS.
...
*  ENDIF.
  ...
  LOOP AT LT_VBRP INTO LS_VBRP.
    ...
    SELECT SINGLE matnr matkl mtart satnr size1 color attyp meins
      FROM MARA INTO LS_MARA WHERE MATNR = LS_VBRP-MATNR .
    IF SY-SUBRC IS INITIAL .
