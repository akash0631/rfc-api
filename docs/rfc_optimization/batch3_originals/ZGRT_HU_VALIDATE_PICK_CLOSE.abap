function ZGRT_HU_VALIDATE_PICK_CLOSE.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*    return.
  ...
  if im_store is not initial.
    im_hu = |{ im_hu alpha = in }|.
    select SINGLE werks INTO @DATA(lv_werks) FROM zgrt_ordconf WHERE exidv eq @im_hu.
    select single dwerks,exidv,sap_hu into @data(ls_store)
                         from zwm_exref where dwerks eq @lv_werks and exidv eq @im_hu ."and sap_hu eq @space.
    if sy-subrc is not initial .
