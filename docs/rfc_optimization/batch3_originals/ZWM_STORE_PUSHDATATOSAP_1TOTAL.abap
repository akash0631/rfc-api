FUNCTION ZWM_STORE_PUSHDATATOSAP_1TOTAL.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  DATA : LT_PUSH TYPE STANDARD TABLE OF ZWM_PUSHTOSAP_T .

  DATA :
    LS_PUSH  TYPE ZWM_PUSHTOSAP_T,
  ...
  DATA :
        REF_DATA TYPE REF TO ZWM_ST_PUSHTOSAP_1STOCK .
  ...
  LOOP AT ET_DATA REFERENCE INTO REF_DATA.

*    IF REF_DATA->QUANTITY GT 999.
*      EX_RETURN-MESSAGE  = 'Un expected Qty in file'.
*      EX_RETURN-TYPE = 'E'.
*      RETURN .
*    ENDIF.

    LV_LEN  = STRLEN( REF_DATA->GANDOLA ).
...
*ls_push-matnr = ls_data-article.
  ...
  MODIFY ZWM_PUSHTOSAP_T FROM TABLE LT_PUSH.
