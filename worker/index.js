...
// âââ DOCX Text Extractor (pure JS, no deps) ââââââââââââââââââââââââââââââââââ
...
// âââ Parse RFC spec from text âââââââââââââââââââââââââââââââââââââââââââââââââ
...
async function genController(spec, sapEnv, apiKey) {
  ...
  const outputDesc = spec.outputType==='table'
    ? `Returns TABLE "${spec.outputTableName}" â dynamic metadata loop, skip STRUCTURE/TABLE fields. Response: {Status,Message,Data:{${spec.outputTableName}:[rows]}}`
...
Error handling: RfcAbapException, CommunicationException, Exception â {Status:"E",Message:ex.Message}
Check EX_RETURN after invoke â if TYPE=="E" return error.

MANDATORY SAP CONNECTOR PATTERN â copy exactly, no variations:
...
NEVER use rfcConfigparameters.GetFunction() â that pattern does not exist.
...
// âââ Push controller to GitHub ââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Register DAB entity ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Update Swagger HTML ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
async function updateSwagger(spec, sapEnv, token) {
  ...
  <div style="border:1px solid #e4e8f0;border-radius:10px;padding:16px;margin-bottom:12px;background:#fff;">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap;">
      ...
      <span style="background:${envBg[sapEnv]};border:1px solid ${envColors[sapEnv]};color:${envColors[sapEnv]};padding:2px 9px;border-radius:5px;font-size:11px;font-family:monospace;">${sapEnv.toUpperCase()} Â· ${env.host}</span>
    </div>
    ...
    <div style="margin-top:8px;font-size:11px;color:#64748b;font-family:monospace;">
      â Data Lake: <a href="${DAB_APP_URL}/api/${tbl}" style="color:#4361ee;">${DAB_APP_URL}/api/${tbl}</a>
    </div>
  </div>`;
...
if (allDone) job.status = job.steps.some(s=>s.status==='error') ? 'error' : 'complete';
  ...
  try {
    ...
    await log('parse','done',`${spec.rfcName} Â· ${spec.category}`);
try{const _pj=JSON.parse(await kv.get(jobId)||'{}');_pj.rfcName=spec.rfcName;_pj.rfcApi=IIS_HOST+'/api/'+spec.rfcName;await kv.put(jobId,JSON.stringify(_pj),{expirationTtl:86400});}catch(_){}
    ...
    try {
      ...
      await log('deploy','running',`Build started Â· run #${runId}`);

      // Poll until completed (max ~8 min = 96 Ã 5s)
      let deployed = false;
      for (let i = 0; i < 96; i++) {
        await sleep(5000);
        ...
        if (run.status === 'completed') {
          if (run.conclusion === 'success') {
            await log('deploy','done',`Live â ${IIS_HOST}/api/${spec.rfcName}`);
            deployed = true;
            ...
            throw new Error(`Deployment ${run.conclusion} â see GitHub Actions`);
          }
          break;
        }
...
// âââ Manage Data Lake Columns âââââââââââââââââââââââââââââââââââââââââââââââââ
async function manageColumns(tableName, operations, token) {
  ...
  if (content) {
    ...
    await ghPut('dab-config.json', JSON.stringify(cfg,null,2), sha,
      `[dab] update field mappings for ${tbl} Â· ${actionSummary}`, token);
    dabUpdated = true;
  }
...
// âââ Data Lake Sync Engine ââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Sync Dashboard HTML ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const SYNC_HTML = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>V2 Retail Â· Data Lake Sync</title>
<style>
...
<div class="top">
  <div class="brand"><div class="bdot"></div>
    <div class="bname">V2 Retail Â· <span>Data Lake Sync</span></div>
    <div class="btag">CRON 02:00 IST</div>
  </div>
  <div class="nav" style="display:flex;gap:4px">
    <a href="/">â¡ Deploy RFC</a>
    <a href="/explore">ð Explorer</a>
    <a href="/sync" style="color:#fff;background:rgba(67,97,238,.25);border:1px solid rgba(67,97,238,.4);padding:5px 11px;border-radius:6px">ð Sync</a>
  </div>
</div>

<div class="app">
  <div class="page-title">Data Lake Sync</div>
  <div class="page-sub">v2retail.net:9005 â Azure SQL (via DAB) Â· auto-runs daily 02:00 IST</div>

  <div class="stat-row">
    <div class="stat"><div class="stat-num" id="st-total">â</div><div class="stat-lbl">Registered RFCs</div></div>
    <div class="stat"><div class="stat-num" id="st-ok" style="color:#16a34a">â</div><div class="stat-lbl">Last Sync OK</div></div>
    <div class="stat"><div class="stat-num" id="st-err" style="color:#dc2626">â</div><div class="stat-lbl">Last Sync Errors</div></div>
  </div>
  ...
  <div class="card">
    ...
    <div class="field" style="margin-bottom:12px">
      <label>Request Params (JSON) â leave {} for full pull</label>
    ...
    <button class="btn" onclick="registerJob()">ï¼ Register RFC</button>
  </div>
  ...
  <div class="card">
    ...
    <div id="jobs-container">
      <div class="empty-state"><div class="empty-icon">ð</div><div>Loading jobsâ¦</div></div>
    </div>
    <div class="schedule-note" style="margin-top:12px">
      â° Auto-runs daily at 02:00 IST via Cloudflare CRON Â· Next run pulls fresh data from v2retail.net into Azure SQL
    </div>
  </div>
</div>
...
function renderJobs() {
  ...
  if (!jobs.length) {
    el.innerHTML = '<div class="empty-state"><div class="empty-icon">ð­</div><div>No sync jobs yet â register an RFC above</div></div>';
    return;
  }
  el.innerHTML = '<table class="jobs-table"><thead><tr>'
    ...
    + jobs.map(j => {
      ...
      const lastSync = r.finishedAt
        ...
        : 'â';
      const rows = r.rowsFetched !== undefined ? r.rowsFetched : 'â';
      const paramsStr = Object.keys(j.params||{}).length
        ? JSON.stringify(j.params).slice(0,40)+'â¦' : '(full pull)';
      return \`<tr>
        ...
        <td><div class="actions">
          <button class="btn btn-sm btn-ghost" onclick="runOne('\${j.rfcName}', this)">â¶ Run</button>
          <button class="btn btn-sm btn-danger" onclick="deleteJob('\${j.rfcName}')">â</button>
        </div></td>
      </tr>\`;
    }).join('')
    + '</tbody></table>';
...
async function runOne(rfcName, btn) {
  ...
  const d = await r.json();
  toast(d.status==='ok' ? 'â '+rfcName+' â '+d.rowsFetched+' rows' : 'â ï¸ '+rfcName+': '+(d.error||d.status));
  await load();
...
async function runAll() {
  ...
  btn.disabled=true; btn.innerHTML='<span class="spin"></span> Runningâ¦';
  const r = await fetch('/sync/run-all', {method:'POST'});
  const results = await r.json();
  const ok = results.filter(x=>x.status==='ok').length;
  toast(\`â \${ok}/\${results.length} syncs completed\`);
  await load();
  btn.disabled=false; btn.textContent='â¶ Run All Now';
}
...
// âââ HTML Upload UI âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const HTML = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>V2 Retail Â· RFC Pipeline</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
...
<body>
<div class="top">
  <div class="brand"><div class="bdot"></div><div class="bname">V2 Retail Â· <span>RFC Pipeline</span></div><div class="btag">LIVE</div></div>
  <div class="nav" style="display:flex;align-items:center;gap:4px">
    <a id="tab-deploy" class="tab-link active" href="javascript:void(0)" onclick="showTab('deploy')">â¡ Deploy RFC</a>
    <a id="tab-columns" class="tab-link" href="javascript:void(0)" onclick="showTab('columns')">ðï¸ Manage Columns</a>
    <a href="/swagger" style="margin-left:8px">Swagger â</a>
  </div>
</div>

<div class="app" id="app">

  <!-- ââ DEPLOY RFC TAB ââ -->
  <div id="panel-deploy">
  <div class="hdr">
    <div class="pill"><div class="dot"></div>UPLOAD Â· PARSE Â· DEPLOY Â· DONE</div>
    <h1>SAP RFC <em>â Live API</em></h1>
    <p>upload rfc .docx â get a working rest api in under 2 minutes</p>
  </div>

  <div class="flow">
    <div class="fl">ð Upload .docx</div><div class="arr">â</div>
    <div class="fl">ð¤ Claude parses</div><div class="arr">â</div>
    <div class="fl">âï¸ C# generated</div><div class="arr">â</div>
    <div class="fl">ð¦ GitHub push</div><div class="arr">â</div>
    <div class="fl">ð Live API</div>
  </div>
  ...
  <div id="upload-section">
    <div class="card">
      ...
      <div class="drop" id="drop"
        ...
        <div class="drop-sub" style="margin-top:4px;font-size:10px;color:#94a3b8">.docx Â· .txt Â· .md</div>
      ...
      <div class="file-sel" id="fileSel" style="display:none">
        <span>ð</span><span id="fileName"></span>
      </div>
    </div>
  ...
  <div id="progress-section" style="display:none">
    <div class="card">
      ...
      <div class="steps" id="steps">
        <div class="step" id="s-parse"><div class="step-icon">â</div><div><div>Parse RFC document</div></div></div>
        <div class="step" id="s-controller"><div class="step-icon">â</div><div><div>Generate ASP.NET controller</div></div></div>
        <div class="step" id="s-github"><div class="step-icon">â</div><div><div>Push to GitHub</div></div></div>
        <div class="step" id="s-deploy"><div class="step-icon">â</div><div><div>Build &amp; Deploy to IIS (.36)</div></div></div>
        <div class="step" id="s-dab"><div class="step-icon">â</div><div><div>Register in Azure DAB</div></div></div>
        <div class="step" id="s-swagger"><div class="step-icon">â</div><div><div>Update Swagger docs</div></div></div>
      </div>
    </div>
  </div>
  ...
  <!-- ââ MANAGE COLUMNS TAB ââ -->
  <div id="panel-columns" style="display:none">
    <div class="hdr">
      <div class="pill"><div class="dot"></div>TABLE Â· ADD / REMOVE COLUMNS Â· PUSH SQL</div>
    ...
    <div id="col-form">
      <div class="card">
        ...
        <div style="font-size:10.5px;color:var(--muted);font-family:var(--mono);margin-top:6px">SQL table name in the data lake â auto-prefixes ET_ if needed</div>
      </div>
    ...
    <div id="col-result" style="display:none">
      <div class="card">
        <div class="ct">Migration SQL â Run on Azure SQL Server</div>
        <div class="sql-out" id="sqlOut"></div>
        <button class="copy-btn" onclick="copySql()">ð Copy SQL</button>
...
async function submitColumns() {
  ...
  try {
    ...
    document.getElementById('colBtn').innerHTML='â¡ Generate SQL &amp; Push to GitHub';
  }
}
...
function showColResult(d, operations) {
  ...
  operations.forEach(op=>{
    html+=(op.action==='ADD'?'<span style="color:var(--green)">â ADD</span>':'<span style="color:var(--red)">ðï¸ REMOVE</span>')+' <b>'+op.column+'</b>'+(op.action==='ADD'?' <span style="color:var(--muted)">'+op.sqlType+'</span>':'')+'<br>';
  });
  if(d.commitUrl) html+='<br>ð¦ <a href="'+d.commitUrl+'" target="_blank" style="color:var(--accent)">View GitHub commit â</a><br>';
  if(d.dabUpdated) html+='ð DAB config updated in GitHub<br>';
...
function copySql(){
  navigator.clipboard.writeText(document.getElementById('sqlOut').textContent)
    .then(()=>{const b=document.querySelector('.copy-btn');b.textContent='â Copied!';setTimeout(()=>b.textContent='ð Copy SQL',2000);});
}
...
function updateStep(id, status, detail) {
  ...
  icon.innerHTML = status==='running'?'<span class="spin"/>':status==='done'?'â':status==='error'?'â':'â';
...
function showResult(job) {
  ...
  chips.innerHTML = '';
  if (job.commit) chips.innerHTML += '<a class="chip g" href="'+job.commit+'" target="_blank">â GitHub commit</a>';
  if (job.dataLake) chips.innerHTML += '<a class="chip b" href="'+job.dataLake+'" target="_blank">ð¡ Data Lake API</a>';
  if (job.swagger) chips.innerHTML += '<a class="chip b" href="'+job.swagger+'" target="_blank">ð Swagger UI</a>';
}
...
function reset() {
  ...
  ['parse','controller','github','deploy','dab','swagger'].forEach(s=>{
    ...
    if(el){el.className='step';el.querySelector('.step-icon').innerHTML='â';const d=el.querySelector('.step-detail');if(d)d.remove();}
  });
}
</script>
</body>
...
// âââ Swagger redirect page ââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
<p>â Redirecting to Swagger UI...</p>
</body></html>`;
...
export default {
  async fetch(request, env, ctx) {
   ...
   const match = url.pathname.match(/^\/(?:pipeline\/)?status\/(.+)$/);
    ...
    // POST /columns â manage data lake columns
    ...
    // ââ SYNC ROUTES âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

    // GET /sync â dashboard UI
    if (url.pathname === '/sync' && request.method === 'GET') {
      return new Response(SYNC_HTML, {headers:{'Content-Type':'text/html;charset=utf-8'}});
    }

    // POST /sync/register â add a new sync job
    ...
    // GET /sync/jobs â list all jobs with last results
    ...
    // POST /sync/run/:rfcName â queue trigger in KV (IIS polls and picks up)
    ...
    if (syncRunMatch && request.method === 'POST') {
      ...
      if (!raw) return new Response(JSON.stringify({error:'Job not found'}),
        {status:404, headers:{'Access-Control-Allow-Origin':'*','Content-Type':'application/json'}});
      // Set a trigger key â IIS SyncController /api/Sync/poll picks this up within 1 min
      await env.RFC_JOBS.put(`sync_trigger:${rfcName}`,
        new Date().toISOString(), { expirationTtl: 300 });
      return new Response(JSON.stringify({
        status: 'queued',
        rfcName,
        msg: 'Sync queued â IIS will execute within 1 minute via poll'
      }), {headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});
    }

    // POST /sync/run-all â queue all triggers
    if (url.pathname === '/sync/run-all' && request.method === 'POST') {
      ...
      return new Response(JSON.stringify({
        ...
        msg: 'All jobs queued â IIS will execute within 1 minute via poll'
      }), {headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});
    }

    // DELETE /sync/delete/:rfcName â remove job
...
<title>V2 Retail Â· RFC API Explorer</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:#f4f6fb;
...
<div class="topbar">
  <div class="logo">
    ...
    <div class="logo-text">V2 Retail <span>Â·</span> RFC Explorer</div>
    <div class="logo-badge">IIS â SAP</div>
  </div>
  <div class="topbar-right">
    <div class="stat"><strong id="tc">â</strong>&nbsp;endpoints</div>
    <div class="stat"><strong id="gc">â</strong>&nbsp;modules</div>
    <div class="live-pill">â LIVE</div>
  </div>
</div>
  ...
  <div class="main">
    <div id="welcome">
      <div class="welcome">
        ...
        <p>All SAP RFC endpoints from your IIS codebase â parameters, response tables, and data lake column management in one place.</p>
        <div class="welcome-stats">
          <div class="wstat"><span class="num" id="ws1">â</span><div class="lbl">Total RFCs</div></div>
          <div class="wstat"><span class="num" id="ws2">â</span><div class="lbl">Modules</div></div>
          <div class="wstat"><span class="num" id="ws3">â</span><div class="lbl">Parameters</div></div>
          <div class="wstat"><span class="num" id="ws4">â</span><div class="lbl">SAP Tables</div></div>
        </div>
      </div>
    </div>
...
function show(ep) {
  ...
  const paramsHtml = ep.params.length ? \`
    <table class="param-table">
      ...
      \`).join("")}</tbody>
    </table>\` : \`<div class="no-data">No parameters extracted â view source file for details.</div>\`;
  ...
  document.getElementById("detail").innerHTML = \`
    <div class="ep-header">
      ...
      <div class="ep-meta">SAP RFC: <span>\${ep.rfc}</span> &nbsp;Â·&nbsp; Module: <span>\${ep.group}</span></div>
      <div class="ep-route-box">
        <span class="ep-route-base">IIS â SAP /</span><span class="ep-route-path">\${ep.route}</span>
      </div>
      <br>
      <a class="source-link" href="\${GITHUB}\${ep.path}" target="_blank">â View source on GitHub</a>
    </div>
    ...
    <div class="card">
      ...
      <div class="col-body">
        ...
        <div class="col-btns">
          <button class="btn-add" onclick="addOp('ADD')">ï¼ Add Column</button>
          <button class="btn-rem" onclick="addOp('REMOVE')">ï¼ Remove Column</button>
        </div>
        <div id="opsList" class="ops-list"></div>
        <button class="btn-apply" id="applyBtn" onclick="applyOps()" disabled>â¡ Apply to Data Lake</button>
...
function renderOps() {
  ...
  el.innerHTML = colOps.map((op,i)=>\`
    <div class="op-row">
      <span class="\${op.action==="ADD"?"op-add":"op-rem"}">\${op.action==="ADD"?"ï¼":"ï¼"}</span>
      <span class="op-col">\${op.column}</span>
      <span class="op-type">\${op.action==="ADD"?op.sqlType:""}</span>
      <button class="op-del" onclick="remOp(\${i})">â</button>
    </div>\`).join("");
...
async function applyOps() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      res.textContent=\`â \${colOps.length} change(s) applied to \${tableName}. SQL migration pushed to GitHub.\`;
      colOps=[]; renderOps();
    } else throw new Error(d.error||"Unknown error");
  } catch(e) {
    res.className="result-box result-err";
    res.textContent="â "+e.message;
  }
  res.style.display="block";
  btn.textContent="â¡ Apply to Data Lake";
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>V2 Retail Â· SAP Fetch</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
...
<body>
<div class="top">
  <div class="brand"><div class="bdot"></div>V2 Retail Â· SAP Fetch <span class="badge">claudetestv2 @ 192.168.151.36</span></div>
  <div style="color:#94a3b8;font-size:12px">Relay: v2-rfc-relay.azurewebsites.net</div>
</div>

<div class="main">
  <h2>SAP RFC â SQL Fetch</h2>
    ...
    <div class="grid2">
      <div>
        ...
        <select id="rfcSelect" onchange="onRfcChange()">
          <optgroup label="ââ Finance ââ">
            <option value="ZADVANCE_PAYMENT_RFC" data-table="ET_ZADVANCE_PAYMENT"
              ...
              ZADVANCE_PAYMENT_RFC â Advance Payment Documents
            </option>
            <option value="ZVND_GATELOT2_PALETTE_VAL_RFC" data-table="ET_PALL">ZVND_GATELOT2_PALETTE_VAL_RFC — Palette Validation GATELOT2</option>
            <option value="ZVND_GATELOT2_PICKLIST_VAL_RFC" data-table="ET_DATA">ZVND_GATELOT2_PICKLIST_VAL_RFC — Picklist Validation GATELOT2</option>
            <option value="ZVND_PUT01_HU_VAL_RFC" data-table="ET_DATA">ZVND_PUT01_HU_VAL_RFC — HU Validation PUT01</option>
            <option value="ZVND_PUT01_SAVE_DATA_RFC" data-table="EX_RETURN">ZVND_PUT01_SAVE_DATA_RFC — Save Data PUT01 [WRITE]</option>
          </optgroup>
          <optgroup label="ââ Gate Entry / LOT Putway ââ">
            <option value="ZVND_PUTWAY_BIN_VAL_RFC" data-table="ET_ZVND_PUTWAY_BIN_VAL"
              ...
              ZVND_PUTWAY_BIN_VAL_RFC â BIN Validation
            </option>
          ...
          <optgroup label="ââ Inbound ââ">
          ...
          <optgroup label="ââ DC Routing ââ">
          ...
          <optgroup label="ââ Vehicle Loading ââ">
          ...
          <optgroup label="ââ HU Scan ââ">
          ...
          <optgroup label="ââ HU Creation ââ">
          ...
          <optgroup label="ââ Vendor SRM Routing ââ">
          ...
          <optgroup label="ââ Finance (Other) ââ">
  ...
  <button class="btn" id="fetchBtn" onclick="doFetch()">â¡ Fetch from SAP â Store in SQL</button>
...
async function doFetch() {
  ...
  try {
    ...
    if (!r.ok || d.error) {
      ...
      res.textContent = 'â ' + (d.error || JSON.stringify(d));
      res.style.display='block';
    } else {
      ...
      res.textContent = 'â Fetched '+d.fetched+' rows from SAP, stored '+d.stored+' in claudetestv2.'
        + (d.truncated ? ' (SAP had more â increase Max Rows to get all)' : '')
    ...
    res.className='result err';
    res.textContent='â Network error: '+e.message;
    res.style.display='block';
  }

  btn.disabled=false; btn.textContent='â¡ Fetch from SAP â Store in SQL';
}
