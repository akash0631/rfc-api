...
{
    /// <summary>
    /// Caches RFC_MASTER + RFC_PARAM from Snowflake GOLD at startup.
    /// Every row in RFC_MASTER becomes a callable API endpoint — zero code deploy needed.
    /// Refreshed on demand via POST /api/catalog/refresh.
    /// Auto-refreshes every 30 minutes on first use.
    /// </summary>
    public class EndpointRegistryService
    {
        ...
        /// <summary>Reload RFC catalog from GOLD.RFC_MASTER + GOLD.RFC_PARAM.</summary>
        public string Refresh()
        {
            lock (_lock)
            {
                ...
                try
                {
                    // STATUS = 'Active' is sufficient — no boolean IS_DELETED filter
                    // (Snowflake.Data .NET 2.x BOOLEAN literal handling varies; skip it)
                    ...
                    foreach (var rfc in rfcs)
                    {
                        ...
                        // RFC_PARAM actual columns: PARAM_NAME, PARAM_TYPE, DATA_TYPE,
                        // DEFAULT_EXPRESSION, IS_REQUIRED, DISPLAY_NAME, SORT_ORDER
                        var paramRows = _sf.QueryAsList(
                            "SELECT PARAM_NAME, PARAM_TYPE, DATA_TYPE, DEFAULT_EXPRESSION, " +
                            "IS_REQUIRED, DISPLAY_NAME, SORT_ORDER " +
