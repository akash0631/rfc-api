...
{
    /// <summary>
    /// RfcDocUploadController - Upload RFC docs, generate C# controllers, push to Finaltest, update Swagger.
    /// Routes:
    ///   GET  /RfcDocUpload          -> Index()      Upload portal UI (startup page)
    ///   POST /RfcDocUpload/Upload   -> Upload()     Parse .docx/.txt/.md, generate C# controller
    ///   GET  /RfcDocUpload/Check    -> CheckFile()  Check if controller exists on Finaltest branch
    ///   POST /RfcDocUpload/Push     -> Push()       Push controller to Finaltest + update Swagger
    ...
    public class RfcDocUploadController : System.Web.Mvc.Controller
    {
        ...
        private const string GH_BRANCH   = "Finaltest";  // Target branch for all pushes
        ...
        public async Task<ActionResult> Upload(HttpPostedFileBase file, string folder = "Finance", string env = "dev")
        {
            ...
            try {
                ...
                return Json(new { success = true, rfcName, folder, filePath = path, controllerCode = code, description = desc,
        ...
        private string BuildSwaggerSpec(string rfcName, string folder, string desc) {
            var spec = new JObject {
                ["openapi"] = "3.0.1",
                ["info"] = new JObject { ["title"] = "V2 Retail · " + rfcName, ["version"] = "1.0.0", ["description"] = desc + " | Branch: Finaltest" },
