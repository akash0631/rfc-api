...
namespace Vendor_SRM_Routing_Application.Controllers
{
    public class RfcDocUploadController : System.Web.Mvc.Controller
    {
        ...
        // Token read from Web.config AppSettings - never in source code
        ...
        public async Task<ActionResult> Upload(HttpPostedFileBase file, string folder = "Finance", string env = "dev")
        {
            ...
            if (Array.IndexOf(allowed, ext) < 0)
                return Json(new { success = false, error = "Use .docx, .txt, or .md" });
            try
            {
                ...
                string docContent = (ext == "txt" || ext == "md")
                    ...
                    : await SendToWorker(bytes, file.FileName);
        ...
        public async Task<ActionResult> Push(RfcPushRequest req)
        {
            ...
            try
            {
                using (var client = GHClient())
                {
                    ...
                    {
                        success      = true,
                        action       = string.IsNullOrEmpty(sha) ? "created" : "updated",
                        commitSha    = result.CommitSha,
                        commitUrl    = "https://github.com/" + GH_REPO + "/commit/" + result.FullSha,
                        fileUrl      = "https://github.com/" + GH_REPO + "/blob/" + GH_BRANCH + "/" + req.FilePath,
                        swaggerUrl   = Url.Action("SwaggerJson", "RfcDocUpload",
                                           new { rfcName = req.RfcName, folder = req.Folder, desc = req.Description },
                                           Request.Url.Scheme),
                        swaggerEntry = swagger,
                        message      = "Controller pushed to Finaltest + Swagger updated!"
                    });
                }
            }
            catch (Exception ex) { return Json(new { success = false, error = ex.Message }); }
        ...
        private async Task<string> UpdateSwaggerEntry(HttpClient client, string rfcName, string folder, string desc, string controllerPath)
        {
            try
            {
                ...
                var pushResult = await PushFile(client, filePath, updated,
                    "feat: register " + rfcName + " in Swagger (Finaltest)", existingSha);
        ...
        private string ExtractDescription(string rfcName, string docContent)
        {
            ...
            {
                var lines = docContent.Split(new string[] { "\n", "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            var spec = new JObject { ["openapi"] = "3.0.1", ["info"] = new JObject { ["title"] = "V2 Retail - " + rfcName, ["version"] = "1.0.0", ["description"] = desc }, ["servers"] = new JArray { new JObject { ["url"] = IIS_BASE } }, ["paths"] = new JObject { ["/api/" + rfcName] = new JObject { ["post"] = new JObject { ["tags"] = new JArray(folder), ["operationId"] = rfcName, ["summary"] = desc, ["requestBody"] = new JObject { ["required"] = true, ["content"] = new JObject { ["application/json"] = new JObject { ["schema"] = new JObject { ["type"] = "object", ["properties"] = new JObject { ["IM_DATA"] = new JObject { ["type"] = "array", ["items"] = new JObject { ["type"] = "object" } } } } } } }, ["responses"] = new JObject { ["200"] = new JObject { ["description"] = "Success" }, ["400"] = new JObject { ["description"] = "SAP Error" } } } } } };
