    ...
    public class RfcDocUploadController : System.Web.Mvc.Controller
    {
        ...
        public async Task<ActionResult> Upload(HttpPostedFileBase file, string folder = "Finance", string env = "dev")
        {
            ...
            try {
                ...
                string docContent = (ext == ".txt" || ext == ".md") ? Encoding.UTF8.GetString(bytes) : await SendToWorkerForParsing(bytes, file.FileName);
                ...
                var desc2   = ExtractDescription(rfcName, docContent);
                return Json(new { success = true, rfcName, folder, filePath = path, controllerCode = code, description = desc2,
                    swaggerUrl = Url.Action("SwaggerJson", "RfcDocUpload", new { rfcName, folder, desc = desc2 }, Request.Url.Scheme),
        ...
        public async Task<ActionResult> CheckFile(string filePath, string token)
        {
            ...
            try { using (var c = GHClient(token)) { var r = await c.GetAsync(GH_API_BASE + "/repos/" + GH_REPO + "/contents/" + filePath + "?ref=" + GH_BRANCH);
                if (r.IsSuccessStatusCode) { var d = Newtonsoft.Json.JsonConvert.DeserializeObject<Newtonsoft.Json.Linq.JObject>(await r.Content.ReadAsStringAsync()); return Json(new { exists = true, sha = d?["sha"]?.ToString() }, JsonRequestBehavior.AllowGet); }
                return Json(new { exists = false }, JsonRequestBehavior.AllowGet); } } catch (Exception ex) { return Json(new { exists = false, error = ex.Message }, JsonRequestBehavior.AllowGet); }
        }
            ...
            try {
                using (var client = GHClient(req.Token)) {
                    ...
                    return Json(new { success = true, action = string.IsNullOrEmpty(sha) ? "created" : "updated",
                        ...
                        swaggerEntry = swaggerMsg, message = "Controller pushed + Swagger updated!" });
                }
            } catch (Exception ex) { return Json(new { success = false, error = ex.Message }); }
        }
        ...
        private string BuildSwaggerSpec(string rfcName, string folder, string desc) {
            var spec = new Newtonsoft.Json.Linq.JObject {
                ["openapi"] = "3.0.1",
                ["info"] = new Newtonsoft.Json.Linq.JObject { ["title"] = "V2 Retail · " + rfcName, ["version"] = "1.0.0", ["description"] = desc },
                ["servers"] = new Newtonsoft.Json.Linq.JArray { new Newtonsoft.Json.Linq.JObject { ["url"] = IIS_BASE, ["description"] = "IIS Primary" } },
                ["paths"] = new Newtonsoft.Json.Linq.JObject {
                    ["/api/" + rfcName] = new Newtonsoft.Json.Linq.JObject {
                        ["post"] = new Newtonsoft.Json.Linq.JObject {
                            ["tags"] = new Newtonsoft.Json.Linq.JArray(folder), ["operationId"] = rfcName, ["summary"] = desc,
                            ["requestBody"] = new Newtonsoft.Json.Linq.JObject { ["required"] = true,
                                ["content"] = new Newtonsoft.Json.Linq.JObject { ["application/json"] = new Newtonsoft.Json.Linq.JObject {
                                    ["schema"] = new Newtonsoft.Json.Linq.JObject { ["type"] = "object",
                                        ["properties"] = new Newtonsoft.Json.Linq.JObject { ["IM_DATA"] = new Newtonsoft.Json.Linq.JObject { ["type"] = "array",
                                            ["items"] = new Newtonsoft.Json.Linq.JObject { ["type"] = "object" } } } } } } },
                            ["responses"] = new Newtonsoft.Json.Linq.JObject {
                                ["200"] = new Newtonsoft.Json.Linq.JObject { ["description"] = "Success" },
                                ["400"] = new Newtonsoft.Json.Linq.JObject { ["description"] = "SAP Error" },
                                ["500"] = new Newtonsoft.Json.Linq.JObject { ["description"] = "Server Error" } }
                        }
                    }
                }
            };
        ...
        private async Task<string> GetFileSha(HttpClient client, string filePath) {
            try { var r = await client.GetAsync(GH_API_BASE + "/repos/" + GH_REPO + "/contents/" + filePath + "?ref=" + GH_BRANCH);
                if (r.IsSuccessStatusCode) { var d = Newtonsoft.Json.JsonConvert.DeserializeObject<Newtonsoft.Json.Linq.JObject>(await r.Content.ReadAsStringAsync()); return d?["sha"]?.ToString(); }
            } catch { } return null;
        }

        private async Task<PushResult> PushFile(HttpClient client, string filePath, string content, string message, string sha) {
            var payload = new Newtonsoft.Json.Linq.JObject { ["message"] = message, ["content"] = Convert.ToBase64String(Encoding.UTF8.GetBytes(content)), ["branch"] = GH_BRANCH };
            ...
            if (!r.IsSuccessStatusCode) { var err = Newtonsoft.Json.JsonConvert.DeserializeObject<Newtonsoft.Json.Linq.JObject>(body); return new PushResult { Success = false, Error = err?["message"]?.ToString() ?? "Push failed" }; }
            var data = Newtonsoft.Json.JsonConvert.DeserializeObject<Newtonsoft.Json.Linq.JObject>(body);
        ...
        private async Task<string> UpdateSwaggerEntry(HttpClient client, string rfcName, string folder, string desc, string controllerPath) {
            try {
                ...
                var data = Newtonsoft.Json.JsonConvert.DeserializeObject<Newtonsoft.Json.Linq.JObject>(await r.Content.ReadAsStringAsync());
                ...
                var pushResult = await PushFile(client, filePath, updated, "feat: register " + rfcName + " in Swagger explorer", existingSha);
                return pushResult.Success ? "Swagger entry added (commit: " + pushResult.CommitSha + ")" : "Swagger update failed: " + pushResult.Error;
            } catch (Exception ex) { return "Swagger error: " + ex.Message; }
        }

        private HttpClient GHClient(string token) {
            var c = new HttpClient(); c.DefaultRequestHeaders.Add("Authorization", "token " + token);
            c.DefaultRequestHeaders.Add("User-Agent", "V2-RFC-Portal/1.0"); c.DefaultRequestHeaders.Add("Accept", "application/vnd.github.v3+json"); return c;
        }

        private async Task<string> SendToWorkerForParsing(byte[] bytes, string fileName) {
            try { using (var client = new HttpClient()) using (var form = new MultipartFormDataContent()) {
                var fc = new ByteArrayContent(bytes); fc.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/octet-stream");
                form.Add(fc, "file", fileName); form.Add(new StringContent("parse"), "mode");
                var r = await client.PostAsync(WORKER_URL + "/parse", form); if (r.IsSuccessStatusCode) return await r.Content.ReadAsStringAsync();
