...
{
    public class ZPF_MASTER_POST_RFCController : BaseController
    {
        

        public async Task<HttpResponseMessage> POST([FromBody] ZPF_MASTER_POST_RFCRequest request)
        {
            ZPF_MASTER_POST_RFC Authenticate = new ZPF_MASTER_POST_RFC();
            ...
            try
            {
                ...
                if (request.IM_ST_CD != "" && request.IM_ST_CD != "")
                {
                    try
                    {
                        ...
                        myfun = rfcrep.CreateFunction("ZPF_MASTER_POST_RFC"); //RfcFunctionName
                                                                                //IRfcStructure rfcFields = myfun.GetStructure("LS_GET_DATA");
                        myfun.SetValue("IM_ST_CD", request.IM_ST_CD);
                        myfun.SetValue("IM_MIN_WAG", request.IM_MIN_WAG);
                        myfun.SetValue("IM_STATUS", request.IM_STATUS);
                        myfun.SetValue("IM_ST_PF_CD", request.IM_ST_PF_CD);
                        myfun.SetValue("IM_ST_PF_CD_REF", request.IM_ST_PF_CD_REF);
    ...
    public class ZPF_MASTER_POST_RFC
    {
        public Boolean Status { get; set; }
        public string Message { get; set; }
        public string StoreId { get; set; }
        public List<ZPF_MASTER_POST_RFCResponse> Data;
        public ZPF_MASTER_POST_RFC()
        {
            Data = new List<ZPF_MASTER_POST_RFCResponse>();
        }
    }
    public class ZPF_MASTER_POST_RFCRequest
    {
        public string IM_ST_CD { get; set; }
        public string IM_MIN_WAG { get; set; }
        public string IM_STATUS { get; set; }
        public string IM_ST_PF_CD { get; set; }
        public string IM_ST_PF_CD_REF { get; set; }



    }
    public class ZPF_MASTER_POST_RFCResponse
    {
