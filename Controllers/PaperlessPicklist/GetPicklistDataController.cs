...
{
    public class GetPicklistDataController : BaseController
    {
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] GetPicklistDataRequest request)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    if (request.IM_USER != null && request.IM_WERKS != null)
                    {
                        ...
                        myfun = rfcrep.CreateFunction("ZWM_RFC_GET_PICKLIST_DATA");
