            ...
            try
            {
                ...
                if (request.IM_OUTPUT != null && request.IM_OUTPUT.Count > 0)
                {
                    ...
                    foreach (var outputItem in request.IM_OUTPUT)
                    {
                        ...
                        if (outputItem.MENGE.HasValue)
                            outputRow.SetValue("MENGE", outputItem.MENGE.Value);
                        if (outputItem.NETPR.HasValue)
                            outputRow.SetValue("NETPR", outputItem.NETPR.Value);
                        ...
                        if (!string.IsNullOrEmpty(outputItem.TYPE))
                            outputRow.SetValue("TYPE", outputItem.TYPE);
