...
{
    public class PO_Fab_POController : BaseController
    {
                ...
                try
                {
                    ...
                    if (request.IM_PO != "" && request.IM_PO != null)
                    {
                        try
                        {
                            ...
                            myfun = rfcrep.CreateFunction("ZSRM_RTL_FAB_PO"); //RfcFunctionName
                                                                              //myfun.SetValue("IM_USER", request.IM_PO); //Import Parameter
                            myfun.SetValue("IM_EBELN", request.IM_PO); //Import Parameter
                            ...
                            else
                            {
                                if (IrfTable.RowCount > 0)
                                {
                                    for (int i = 0; i < IrfTable.RowCount; ++i)
                                    {
                                        ...
                                        authenticateResponse.PO_NO = IrfTable[i].GetString("SUBMI");
