...
{
    public class Validation_BINController : BaseController
    {
        // GET: HUNameCheck
        [b.HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] Validate_BINRequest request)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    if (request.IM_SITE != "" && request.IM_SITE != null &&
                        request.IM_BIN != "" && request.IM_BIN != null &&
                        request.IM_LGTYP != "" && request.IM_LGTYP != null
                        )
                    {
                        ...
                        myfun = rfcrep.CreateFunction("ZFMS_RFC_FABPUTWAYGRC_BIN_VALI"); //RfcFunctionName
                                                                                         //IRfcStructure E_Data = myfun.GetStructure("ET_DATA");
                        myfun.SetValue("IM_SITE", request.IM_SITE);
                        myfun.SetValue("IM_BIN", request.IM_BIN);
                        myfun.SetValue("IM_LGTYP", request.IM_LGTYP);
