...
namespace Vendor_SRM_Routing_Application.Controllers.Finance
{
    ...
    {
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] ZWM_HU_MVT_BIN_VAL_RFCRequest request)
        {
            return await Task.Run(() =>
            {
                try {
                    ...
                    IRfcFunction myfun = dest.Repository.CreateFunction("ZWM_HU_MVT_BIN_VAL_RFC");
    ...
    public class ZWM_HU_MVT_BIN_VAL_RFCRequest { public List<Dictionary<string,string>> IM_DATA { get; set; } }
}