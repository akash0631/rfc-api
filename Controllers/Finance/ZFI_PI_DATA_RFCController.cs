...
namespace Vendor_SRM_Routing_Application.Controllers.Finance
{
    ...
    {
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] ZFI_PI_DATA_RFCRequest request)
        {
            return await Task.Run(() =>
            {
                try {
                    ...
                    IRfcFunction myfun = dest.Repository.CreateFunction("ZFI_PI_DATA_RFC");
    ...
    public class ZFI_PI_DATA_RFCRequest { public List<Dictionary<string,string>> IM_DATA { get; set; } }
}