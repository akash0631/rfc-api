...
namespace Vendor_SRM_Routing_Application.Controllers.Finance
{
    ...
    {
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] ZWM_HU_MVT_SAVE_RFCRequest request)
        {
            return await Task.Run(() =>
            {
                try {
                    ...
                    IRfcFunction myfun = dest.Repository.CreateFunction("ZWM_HU_MVT_SAVE_RFC");
    ...
    public class ZWM_HU_MVT_SAVE_RFCRequest { public List<Dictionary<string,string>> IM_DATA { get; set; } }
}