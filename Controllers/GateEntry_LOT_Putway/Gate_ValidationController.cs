...
{
    public class Gate_ValidationController : BaseController
    {
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] Gate_ValidationRequest request)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    if (request.IM_USER != null && request.IM_WERKS != null)
                    {
                        ...
                        myfun = rfcrep.CreateFunction("ZWM_GATE_VALIDATION1_N");
    ...
    public class Gate_ValidationRequest
    {
