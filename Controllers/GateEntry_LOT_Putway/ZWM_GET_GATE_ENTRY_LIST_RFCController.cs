...
{
    public class ZWM_GET_GATE_ENTRY_LIST_RFCController : BaseController
    {
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] ZWM_GET_GATE_ENTRY_LIST_RFCRequest request)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    if ( request.IM_WERKS != null)
                    {
                        ...
                        myfun = rfcrep.CreateFunction("ZWM_GET_GATE_ENTRY_LIST_RFC");

                        myfun.SetValue("IM_USER", request.IM_USER);
                        myfun.SetValue("IM_WERKS", request.IM_WERKS);
                        myfun.SetValue("IM_DOCNO", request.IM_DOCNO);
    ...
    public class ZWM_GET_GATE_ENTRY_LIST_RFCRequest
    {
        public string IM_USER { get; set; }
        public string IM_WERKS { get; set; }
        public string IM_DOCNO { get; set; }
