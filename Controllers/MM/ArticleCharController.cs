    ...
    public class ArticleCharController : BaseController
    {
        ...
        public HttpResponseMessage Patch([FromBody] ArticlePatchRequest request, string env = "dev")
        {
            ...
            try
            {
                ...
                // chain for any KLART), scans every KSML per attr, picks first hosting
                // class, groups BAPI calls by (CLASSTYPE, CLASS). CRITICAL:
                // BAPI_OBJCL_CHANGE is always invoked with OBJECTKEY = MATNR even for
                // KLART 026 - BAPI resolves CUOBJ internally via INOB. Passing CUOBJ
                // directly returns CL/763 "Object does not exist".
                // Routes per attr:
