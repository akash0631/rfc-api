...
namespace Vendor_Application_MVC.Controllers
{
    ...
    public class NSOSiteList
    {
        public Boolean Status { get; set; }
        public string Message { get; set; }
        public List<NSOSiteListResponse> Data;
        public NSOSiteList()
        {
            Data = new List<NSOSiteListResponse>();
        }
    }

    public class NSOSiteListResponse
    {
        ...
        public string ZSNRO { get; set; } = String.Empty;
        

    }
    public class NSOSiteListController : BaseController
    {
        [EnableCors(origins: "*", headers: "*", methods: "*")]
        [HttpPost]
        public async Task<HttpResponseMessage> Post()
        {
            NSOSiteList PO_Detail = new NSOSiteList();
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    myfun = rfcrep.CreateFunction("ZNSO_RFC_SITELIST"); //RfcFunctionName
                    ...
                    if (SAP_TYPE == "E")
                    {
                        ...
                        return Request.CreateResponse<List<NSOConfigRoutingResponse>>(HttpStatusCode.BadRequest, new List<NSOConfigRoutingResponse>());
                    }
                    else
                    {

                        List<NSOSiteListResponse> list = new List<NSOSiteListResponse>();

                        for (int i = 0; i < IrfTable.RowCount; ++i)
                        {
                            NSOSiteListResponse obj = new NSOSiteListResponse();
                            //var obj = new NSOSiteListResponse();
                            obj.SITE_CODE = IrfTable[i].GetString("SNRO");
                            obj.WERKS = IrfTable[i].GetString("WERKS");
                            obj.KTEXT = IrfTable[i].GetString("KTEXT");
                            obj.ZSNRO = IrfTable[i].GetString("ZSNRO");
                    ...
                    // return Request.CreateResponse<List<NSOConfigRoutingResponse>>(HttpStatusCode.OK, list);
                ...
                catch (Exception ex)
                {
                    ...
                    // return Request.CreateResponse<List<NSOConfigRoutingResponse>>(HttpStatusCode.InternalServerError, new List<NSOConfigRoutingResponse>());
